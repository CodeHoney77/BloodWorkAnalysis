...
# write.csv(outliers, csv_file_path, row.names = FALSE)
# Visualize outliers in a specific metabolite
library(ggplot2)
...
for (metabolite in names(select_if(raw_data_urine_clean, is.numeric))) {
  ...
  if (any(outlier_summary[[summary_column_name]] == TRUE, na.rm = TRUE)) {
    p <- ggplot(outliers, aes_string(x = "day", y = metabolite)) +
      geom_line() +
      geom_point(aes_string(color = outlier_column_name)) +
      geom_text(aes(label = id_and_day), nudge_y = 0.1, hjust = 0.5, vjust = -1, check_overlap = TRUE, color = "black", size = 3) +
      scale_color_manual(values = c("green", "red")) +
      labs(title = paste("Outliers in", metabolite, "Levels Across Days"),
           x = "Day", y = "Metabolite Level")
